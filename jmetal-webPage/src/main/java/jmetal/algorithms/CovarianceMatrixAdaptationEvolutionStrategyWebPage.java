...
public class CovarianceMatrixAdaptationEvolutionStrategyWebPage
	...
	/** Constructor */
	public CovarianceMatrixAdaptationEvolutionStrategyWebPage(ResultRepository resultRepository,
			WebPageExperiment experimentName, Builder builder) {
	...
	/** Constructor */
	private CovarianceMatrixAdaptationEvolutionStrategyWebPage(Builder builder) {
		super(builder.problem);
		this.lambda = builder.lambda;
		this.maxEvaluations = builder.maxEvaluations;
		this.typicalX = builder.typicalX;
	...
	public static class Builder {
		...
		public CovarianceMatrixAdaptationEvolutionStrategyWebPage build() {
			return new CovarianceMatrixAdaptationEvolutionStrategyWebPage(this);
		}
	}
	...
	private void initializeInternalParameters() {
		...
		chiN = Math.sqrt(numberOfVariables)
				* (1 - 1 / (4 * numberOfVariables) + 1 / (21 * numberOfVariables * numberOfVariables));
	...
	private int updateEvolutionPaths(double[] oldDistributionMean) {
		...
		int hsig = 0;
		if ((Math.sqrt(psxps) / Math.sqrt(1. - Math.pow(1. - cumulationSigma, 2. * evaluations / lambda)) / chiN) < (1.4
				+ 2. / (numberOfVariables + 1.))) {
			hsig = 1;
		}
		for (int i = 0; i < numberOfVariables; i++) {
			pathsC[i] = (1. - cumulationC) * pathsC[i] + hsig * Math.sqrt(cumulationC * (2. - cumulationC) * muEff)
		...
		for (int i = 0; i < numberOfVariables; i++) {
			for (int j = 0; j <= i; j++) {
				c[i][j] = (1 - c1 - cmu) * c[i][j]
						+ c1 * (pathsC[i] * pathsC[j] + (1 - hsig) * cumulationC * (2. - cumulationC) * c[i][j]);
				for (int k = 0; k < mu; k++) {
					/*
					...
					c[i][j] += cmu * weights[k] * (valueI - oldDistributionMean[i]) * (valueJ - oldDistributionMean[j])
							/ sigma / sigma;
				}
			}
		}
	...
	private void checkEigenCorrectness() {
		...
		for (int i = 0; i < numberOfVariables; i++) {
			// Numerical problem?
			if (diagD[i] < 0) {
				JMetalLogger.logger.severe("CovarianceMatrixAdaptationEvolutionStrategy.updateDistribution:"
